...
{
    public class WindowPermissionWrapper : IDisposable
    {
        ...
        public WindowPermissionWrapper(Watcher watcher)
        {
            _watcher = watcher;
        ...
        public async Task OnRequest()
        {
            ...
            App.Logger.WriteLine("Getting","SIGMa");

            ShowAlert(
                title,
                String.Format(Strings.ContextMenu_WindowPermission_Notification_Text, universeDetails.Data.Name),
                10,
                (_, _) => _menuContainer.ShowWindowPermissionWindow()
            );
        }
        #endregion
        ...
        public void ShowAlert(string caption, string message, int duration, EventHandler? clickHandler)
        {
            ...
            string LOG_IDENT = $"WindowPermissionWrapper::ShowAlert.{id}";
